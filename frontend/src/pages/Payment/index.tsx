...
export default function Payment() {
  ...
  const createBookingAndUploadReceipt = async () => {
    ...
    try {
      ...
      const receiptErrorMessage = err instanceof Error ? err.message : 'Receipt upload failed. Please try again.'
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.3 }} className="min-h-screen bg-white">
      <div className="flex flex-col px-4 sm:px-12 lg:px-20 py-8 sm:py-12 max-w-4xl mx-auto">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <p className="text-base sm:text-lg text-gray-600">Scan the QR code to pay and upload your receipt</p>
        </div>
        <div className="bg-gray-50 rounded-2xl p-4 sm:p-5 mb-6 sm:mb-8">
          ...
          <div className="space-y-3 text-xs sm:text-sm">
            ...
            <div className="pt-3 border-t flex justify-between"><span className="font-semibold">Total</span><span className="font-bold text-lg sm:text-xl text-brand">{formatPrice(service.price)}</span></div>
          </div>
        </div>
        ...
        {!uploadSuccess && <>
          ...
          <Button onClick={createBookingAndUploadReceipt} className="w-full h-11 sm:h-12 text-sm sm:text-base bg-brand hover:bg-brand-600" disabled={isLoading || !receiptPreview}>{isLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Processing...</> : <><Upload className="mr-2 h-4 w-4" />Submit Receipt</>}</Button>
